import pandas as pd
import numpy as np
import re
from typing import Dict, List, Tuple
from difflib import SequenceMatcher

class EcommerceDataPreprocessor:
    """
    Comprehensive data preprocessing pipeline for multi-platform e-commerce data.
    Handles cleaning, normalization, matching, and price comparison.
    """
    
    def __init__(self, df: pd.DataFrame):
        """Initialize with raw scraped data"""
        self.df = df.copy()
        self.processed_df = None
        self.matched_products = None
        self.price_comparison = None
        
    def clean_data(self) -> pd.DataFrame:
        """Step 1: Clean and standardize the raw data"""
        df = self.df.copy()
        
        # Remove duplicates
        df = df.drop_duplicates()
        
        # Handle missing values
        df['brand'] = df['brand'].fillna('Unknown')
        df['url'] = df['url'].fillna('')
        
        # Clean price data
        df['price_rupees'] = pd.to_numeric(df['price_rupees'], errors='coerce')
        df = df[df['price_rupees'].notna()]  # Remove rows with invalid prices
        
        # Clean weight data
        df['weight_grams'] = pd.to_numeric(df['weight_grams'], errors='coerce')
        
        # Extract weight from pack_display if weight_grams is missing
        df['weight_grams'] = df.apply(
            lambda row: self._extract_weight(row['pack_display']) 
            if pd.isna(row['weight_grams']) else row['weight_grams'], 
            axis=1
        )
        
        # Standardize product names
        df['product_name_clean'] = df['product_name'].apply(self._clean_product_name)
        
        # Standardize brand names
        df['brand_clean'] = df['brand'].str.strip().str.title()
        
        self.processed_df = df
        return df
    
    def _extract_weight(self, pack_display: str) -> float:
        """Extract weight in grams from pack_display string"""
        if pd.isna(pack_display):
            return np.nan
        
        # Look for patterns like "400 g", "1 kg", "200g"
        match = re.search(r'(\d+\.?\d*)\s*(g|kg|ml|l)', str(pack_display), re.IGNORECASE)
        if match:
            value = float(match.group(1))
            unit = match.group(2).lower()
            
            # Convert to grams
            if unit in ['kg', 'l']:
                return value * 1000
            else:
                return value
        return np.nan
    
    def _clean_product_name(self, name: str) -> str:
        """Clean and normalize product name"""
        if pd.isna(name):
            return ''
        
        # Convert to lowercase
        name = str(name).lower()
        
        # Remove special characters but keep spaces
        name = re.sub(r'[^a-z0-9\s]', ' ', name)
        
        # Remove extra whitespace
        name = ' '.join(name.split())
        
        return name
    
    def calculate_unit_price(self) -> pd.DataFrame:
        """Step 2: Calculate price per 100g for comparison"""
        df = self.processed_df.copy()
        
        # Calculate price per 100g
        df['price_per_100g'] = df.apply(
            lambda row: (row['price_rupees'] / row['weight_grams'] * 100) 
            if pd.notna(row['weight_grams']) and row['weight_grams'] > 0 
            else np.nan,
            axis=1
        )
        
        self.processed_df = df
        return df
    
    def _similarity_score(self, str1: str, str2: str) -> float:
        """Calculate similarity between two strings"""
        return SequenceMatcher(None, str1, str2).ratio()
    
    def _extract_keywords(self, name: str) -> set:
        """Extract key product keywords"""
        # Common stop words to remove
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'by', 'with', 'from'}
        
        words = name.split()
        keywords = {w for w in words if w not in stop_words and len(w) > 2}
        
        return keywords
    
    def match_products_across_platforms(self, 
                                       name_threshold: float = 0.7,
                                       brand_match_required: bool = True) -> pd.DataFrame:
        """
        Step 3: Match identical products across different platforms
        
        Args:
            name_threshold: Minimum similarity score for product names (0-1)
            brand_match_required: Whether brands must match exactly
        """
        df = self.processed_df.copy()
        
        # Group by brand and category
        matches = []
        
        for brand in df['brand_clean'].unique():
            brand_df = df[df['brand_clean'] == brand]
            
            # Get products from different platforms
            platforms = brand_df['platform'].unique()
            
            if len(platforms) < 2:
                continue
            
            # Compare products across platforms
            for i, row1 in brand_df.iterrows():
                for j, row2 in brand_df.iterrows():
                    if i >= j:
                        continue
                    
                    # Must be from different platforms
                    if row1['platform'] == row2['platform']:
                        continue
                    
                    # Check brand match
                    if brand_match_required and row1['brand_clean'] != row2['brand_clean']:
                        continue
                    
                    # Calculate name similarity
                    similarity = self._similarity_score(
                        row1['product_name_clean'],
                        row2['product_name_clean']
                    )
                    
                    # Check keyword overlap
                    keywords1 = self._extract_keywords(row1['product_name_clean'])
                    keywords2 = self._extract_keywords(row2['product_name_clean'])
                    keyword_overlap = len(keywords1 & keywords2) / max(len(keywords1), len(keywords2), 1)
                    
                    # Weight matching score
                    combined_score = (similarity * 0.6) + (keyword_overlap * 0.4)
                    
                    # Check weight similarity (within 10% tolerance)
                    weight_match = False
                    if pd.notna(row1['weight_grams']) and pd.notna(row2['weight_grams']):
                        weight_diff = abs(row1['weight_grams'] - row2['weight_grams'])
                        weight_match = weight_diff / max(row1['weight_grams'], row2['weight_grams']) < 0.1
                    
                    # If similarity is high enough and weights match
                    if combined_score >= name_threshold and (weight_match or pd.isna(row1['weight_grams'])):
                        matches.append({
                            'product_name': row1['product_name'],
                            'brand': row1['brand_clean'],
                            'weight_grams': row1['weight_grams'],
                            'platform_1': row1['platform'],
                            'price_1': row1['price_rupees'],
                            'price_per_100g_1': row1['price_per_100g'],
                            'url_1': row1['url'],
                            'platform_2': row2['platform'],
                            'price_2': row2['price_rupees'],
                            'price_per_100g_2': row2['price_per_100g'],
                            'url_2': row2['url'],
                            'similarity_score': combined_score
                        })
        
        self.matched_products = pd.DataFrame(matches)
        return self.matched_products
    
    def compare_prices(self) -> pd.DataFrame:
        """Step 4: Compare prices and identify best deals"""
        if self.matched_products is None or len(self.matched_products) == 0:
            print("No matched products found. Run match_products_across_platforms() first.")
            return pd.DataFrame()
        
        df = self.matched_products.copy()
        
        # Calculate price difference
        df['price_difference'] = df['price_2'] - df['price_1']
        df['price_difference_pct'] = (df['price_difference'] / df['price_1']) * 100
        
        # Calculate unit price difference
        df['unit_price_diff'] = df['price_per_100g_2'] - df['price_per_100g_1']
        df['unit_price_diff_pct'] = (df['unit_price_diff'] / df['price_per_100g_1']) * 100
        
        # Identify cheaper platform
        df['cheaper_platform'] = df.apply(
            lambda row: row['platform_1'] if row['price_1'] < row['price_2'] else row['platform_2'],
            axis=1
        )
        
        df['best_price'] = df[['price_1', 'price_2']].min(axis=1)
        df['savings'] = abs(df['price_difference'])
        
        # Sort by savings potential
        df = df.sort_values('savings', ascending=False)
        
        self.price_comparison = df
        return df
    
    def get_platform_summary(self) -> pd.DataFrame:
        """Generate summary statistics by platform"""
        df = self.processed_df.copy()
        
        summary = df.groupby('platform').agg({
            'product_name': 'count',
            'price_rupees': ['mean', 'median', 'min', 'max'],
            'price_per_100g': ['mean', 'median']
        }).round(2)
        
        summary.columns = ['_'.join(col).strip() for col in summary.columns]
        summary = summary.rename(columns={
            'product_name_count': 'total_products',
            'price_rupees_mean': 'avg_price',
            'price_rupees_median': 'median_price',
            'price_rupees_min': 'min_price',
            'price_rupees_max': 'max_price',
            'price_per_100g_mean': 'avg_price_per_100g',
            'price_per_100g_median': 'median_price_per_100g'
        })
        
        return summary.reset_index()
    
    def get_brand_summary(self) -> pd.DataFrame:
        """Generate summary statistics by brand"""
        df = self.processed_df.copy()
        
        brand_summary = df.groupby('brand_clean').agg({
            'product_name': 'count',
            'platform': lambda x: x.nunique(),
            'price_rupees': 'mean',
            'price_per_100g': 'mean'
        }).round(2)
        
        brand_summary.columns = ['product_count', 'platforms_available', 'avg_price', 'avg_price_per_100g']
        brand_summary = brand_summary.sort_values('product_count', ascending=False)
        
        return brand_summary.reset_index()
    
    def export_for_dashboard(self, output_prefix: str = 'processed'):
        """Export processed data for dashboard visualization"""
        
        # Export main processed data
        if self.processed_df is not None:
            self.processed_df.to_csv(f'{output_prefix}_products.csv', index=False)
            print(f"✓ Exported: {output_prefix}_products.csv")
        
        # Export matched products
        if self.matched_products is not None and len(self.matched_products) > 0:
            self.matched_products.to_csv(f'{output_prefix}_matched.csv', index=False)
            print(f"✓ Exported: {output_prefix}_matched.csv")
        
        # Export price comparison
        if self.price_comparison is not None and len(self.price_comparison) > 0:
            self.price_comparison.to_csv(f'{output_prefix}_price_comparison.csv', index=False)
            print(f"✓ Exported: {output_prefix}_price_comparison.csv")
        
        # Export summaries
        platform_summary = self.get_platform_summary()
        platform_summary.to_csv(f'{output_prefix}_platform_summary.csv', index=False)
        print(f"✓ Exported: {output_prefix}_platform_summary.csv")
        
        brand_summary = self.get_brand_summary()
        brand_summary.to_csv(f'{output_prefix}_brand_summary.csv', index=False)
        print(f"✓ Exported: {output_prefix}_brand_summary.csv")
        
        print("\n✓ All data exported successfully!")
    
    def run_full_pipeline(self, export: bool = True) -> Dict[str, pd.DataFrame]:
        """Run the complete preprocessing pipeline"""
        print("Starting data preprocessing pipeline...\n")
        
        # Step 1: Clean data
        print("Step 1: Cleaning data...")
        self.clean_data()
        print(f"✓ Cleaned {len(self.processed_df)} records\n")
        
        # Step 2: Calculate unit prices
        print("Step 2: Calculating unit prices...")
        self.calculate_unit_price()
        print(f"✓ Calculated unit prices\n")
        
        # Step 3: Match products
        print("Step 3: Matching products across platforms...")
        self.match_products_across_platforms()
        print(f"✓ Found {len(self.matched_products) if self.matched_products is not None else 0} product matches\n")
        
        # Step 4: Compare prices
        print("Step 4: Comparing prices...")
        self.compare_prices()
        print(f"✓ Generated price comparisons\n")
        
        # Export data
        if export:
            print("Step 5: Exporting data...")
            self.export_for_dashboard()
        
        return {
            'processed_data': self.processed_df,
            'matched_products': self.matched_products,
            'price_comparison': self.price_comparison,
            'platform_summary': self.get_platform_summary(),
            'brand_summary': self.get_brand_summary()
        }


# Example usage
if __name__ == "__main__":
    # Load your scraped data
    df = pd.read_csv('data/bread_products.csv')
    
    # Initialize preprocessor
    preprocessor = EcommerceDataPreprocessor(df)
    
    # Run full pipeline
    results = preprocessor.run_full_pipeline(export=True)
    
    # Access results
    print("\n" + "="*50)
    print("PREPROCESSING SUMMARY")
    print("="*50)
    print(f"\nTotal products processed: {len(results['processed_data'])}")
    print(f"Products matched across platforms: {len(results['matched_products'])}")
    print(f"Price comparisons available: {len(results['price_comparison'])}")
    
    # Display top 5 best deals
    if len(results['price_comparison']) > 0:
        print("\n" + "="*50)
        print("TOP 5 BEST DEALS")
        print("="*50)
        top_deals = results['price_comparison'].head(5)[
            ['product_name', 'brand', 'cheaper_platform', 'best_price', 'savings']
        ]
        print(top_deals.to_string(index=False))